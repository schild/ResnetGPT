...
for root, dirs, files in os.walk(操作记录):
    if len(dirs)>0:
        break
for 号 in dirs:
   路径json = f'{操作记录}/{号}/_操作数据.json'
   numpy数组路径 = f'{操作记录}/{号}/图片_操作预处理数据2.npz'
   ...
   with open(路径json, encoding='ansi') as f:
      ...
      for item in 数据列:
         ...
         if 图片张量.shape[0] == 0:
            img = Image.open(f'{操作记录}/{号}' + f'/{df["图片号"]}.jpg')
            img2 = np.array(img)
            ...
            操作序列[0, 0] = 词数词典[f"{移动操作}_" + df["动作操作"]]
         else:
            img = Image.open(f'{操作记录}/{号}' + f'/{df["图片号"]}.jpg')
            img2 = np.array(img)
            ...
            if 移动操作a!='无移动':
                移动操作=移动操作a
            操作序列 = np.append(操作序列, 词数词典[f"{移动操作}_" + df["动作操作"]])
