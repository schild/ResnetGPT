...
for j in range(100):
   random.shuffle(dirs)
   for 号 in dirs:
      预处理数据 = f'../训练数据样本/{号}/图片_操作预处理数据2.npz'
      if os.path.isfile(预处理数据):
         ...
         while 循环:
            if (i+1)*树枝<len(操作_分_表):
               ...
               操作_分_枝 = np.array(操作_分_表[i * 树枝:])
               图片_分_枝 = np.array(图片_分_表[i * 树枝:], dtype=np.float32)
               目标输出_分_枝 = np.array(目标输出_分_表[i * 树枝:])
               循环 = False
            ...
            if 计数 % 1 == 0:
               ...
               print(f"用时{用时} 第{j}轮 第{计数}张 号{号}")
            if 计数 % 45060 == 0:
                print('888')
   ...
   torch.save(model.state_dict(), f'weights/model_weights_P{str(j)}')
